...
- Создаем .dockerignore и Dockerfile. В Dockerfile прописываем выполнение команд для установки пакетов nginx и openssl в контейнер, указываем на необходимость открыть порт 443, копируем конфигурационный файл default в директорию /etc/nginx/sites-enabled и скрипт в любую директорию и описываем команду со скриптом в качестве аргумента (указываем полный путь, в который скопируем скрипт), которую нужно выполнить, когда контейнер будет запущен
